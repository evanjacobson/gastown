...
func (t *Tmux) KillSessionWithProcesses(name string) error {
	...
	if pid != "" {
		...
		if pgid != "" && pgid != "0" && pgid != "1" {
			// Kill process group using killProcessGroup() which uses syscall.Kill()
			// directly on Unix, rather than shelling out to /usr/bin/kill which has
			// parsing ambiguity with negative PGIDs.
			// procps-ng kill (v4.0.4+) misparses "-PGID" and can kill ALL processes.
			// On Windows, killProcessGroup is a no-op (tmux doesn't exist there).
			pgidInt, _ := strconv.Atoi(pgid)
			_ = killProcessGroup(pgidInt, signalSIGTERM)
			time.Sleep(100 * time.Millisecond)
			_ = killProcessGroup(pgidInt, signalSIGKILL)
		}
...
func (t *Tmux) KillPaneProcesses(pane string) error {
	...
	pgid := getProcessGroupID(pid)
	if pgid != "" && pgid != "0" && pgid != "1" {
		// Kill process group using killProcessGroup() which abstracts syscall.Kill().
